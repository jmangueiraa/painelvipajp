      ...
      GET: async ({ request }) => {
        try {
          ...
          const [{ data: payments }, { data: plan }, { data: server }, { data: referrals }, { data: settings }, { data: plans }, { data: updates }] = await Promise.all([
            ...
            supabaseAdmin.from("settings").select("referral_reward_days,referral_enabled,app_android_url,app_ios_url,updates_movies_text,updates_series_text").eq("user_id", client.user_id).maybeSingle(),
            plansQuery,
